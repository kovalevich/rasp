...
$(document).ready(function() {
    ...
    if($('#search').exists()) {
        var search_form = new Form('search');
    ...
    if($('#show-all').exists()){
        $('#show-all').unbind().click(function(){
            $('#box-popular').toggleClass('hidden');
            $('#box-all').toggleClass('hidden');
            return false;
        });
    }

    if($('#show-popular').exists()){
        $('#show-popular').unbind().click(function(){
            $('#box-popular').toggleClass('hidden');
            $('#box-all').toggleClass('hidden');
            return false;
        });
    ...
    function e() {
        $(window).width() < 997 ? $(".left-sidebar").hasClass("minified") && ($(".left-sidebar").removeClass("minified"), $(".left-sidebar").addClass("init-minified")) : $(".left-sidebar").hasClass("init-minified") && $(".left-sidebar").removeClass("init-minified").addClass("minified")
    }
    ...
    $(".top-general-alert").length > 0 && ($(".top-general-alert").delay(100).slideDown("medium"), $(".top-general-alert .close").click(function() {
...
check_user_place = function (user, place) {
    $.ajax({
        type: 'get',
        dataType: 'json',
        url: '/ajax/mod/user',
...
var parse = function ()
{
    $.ajax({
        ...
        beforeSend: function(){
            ...
            $('#form').slideUp();
            $(".top-general-alert").delay(1).slideDown("medium");
        },
        success: function(data){
            if(data.success)
            {
                $(".top-general-alert").delay(1).slideUp("fast");
                $('#result').slideDown();
                $('#dyn-alert').html(
